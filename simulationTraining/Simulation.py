...
class Simulation(object):
    ...
    def __init__(self, args, graphNetwork, train_loaders, test_loaders, models):
        ...
        for i in range(self.graphNetwork.size):
            self.optimizers.append(optim.SGD(self.models[i].parameters(),
                                             lr=self.args.lr,
            ...
            for param in self.models[i].parameters():
                self.tensor_lists[i].append(param.data)
    ...
    def run(self):
        ...
        for epoch in range(self.args.epoch):
            print(f"训练轮数： {epoch:0>2} / {self.args.epoch}")
            self.tensor_lists = [[] for _ in range(self.graphNetwork.size)]
            ...
            for i in range(self.graphNetwork.size):
                ...
                for batch_idx, (data, target) in enumerate(self.train_loaders[i]):
                    ...
                    self.top1s[i].update(acc1[0], data.size(0))
                    if batch_idx % 10 == 0:
                        print(
            ...
            for i in range(self.graphNetwork.size):
                ...
                for j in range(i + 1, self.graphNetwork.size):
                    if self.graphNetwork.matrix[i][j] != -1:
                        # 接收邻居节点传来的模型参数
                        self.receiveModels[j] += sendModel
            ...
            for i in range(self.graphNetwork.size):
                ...
                print(f"{epoch:0>2}  ID: %d, epoch: %.3f, loss: %.3f, train_acc: %.3f, test_acc: %.3f" % (i, epoch,
                                                                                                          self.losses[
                                                                                                              i].avg,
                                                                                                          self.top1s[
                                                                                                              i].avg,
